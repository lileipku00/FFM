...
m_da_all = np.array([])
co_da_all = np.array([])
for _i in range(len(passed_staev[0])):
    flag = 'T'
    x_sta = np.array([])
    dt_sta = np.array([])
    da_sta = np.array([])
    for _j in range(len(bands)):
        if passed_staev[_j][_i][4] < xcorr_limit:
            flag = 'F'
    if flag == 'F': continue
    for _j in range(len(bands)):
        x_sta = np.append(x_sta, passed_staev[_j][_i][5])
        dt_sta = np.append(dt_sta, passed_staev[_j][_i][2])
        # divided by 1.e9 because of the difference in scale (real and syn)
        da_sta = np.append(da_sta, passed_staev[_j][_i][3]/1.e9)
